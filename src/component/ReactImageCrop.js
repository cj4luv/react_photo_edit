...
class ReactImageCrop extends Component {
    ...
    componentDidMount(){
        ...
        image.onload = function () {
            ...
            if (parseInt(block.style.width.replace('px', ''),10) > a.refs.RICC_image.width) w = a.refs.RICC_image.width;
            if (parseInt(block.style.height.replace('px', ''),10) > a.refs.RICC_image.height) h = a.refs.RICC_image.height;
        ...
        if(!this.state.activeCropBlock){
            ...
            if(e.target===bg_block){
                ...
                let divTop = crop_block.style.top.replace('px',''),
                    ...
                    eWi = parseInt(this.state.cropSize.w,10),
                    eHe = parseInt(this.state.cropSize.h,10),
                    cWi = parseInt(main_block.style.width,10),
                    cHe = parseInt(main_block.style.height,10),
                ...
                if(this.props.resize){
                    ...
                    this.setState({
                        ...
                        resize: {
                            w: parseInt(crop_block.style.width.replace('px', ''),10),
                            h: parseInt(crop_block.style.height.replace('px', ''),10)
                        }
                    });
                }
            }
        ...
        if (this.state.status){
            ...
            if(!this.state.activeCropBlock){
                let startX = this.state.start.x,
                    ...
                    bWi = parseInt(main_block.style.width.replace('px', ''),10),
                    bLeft = this.state.blockX,
                    bHe = parseInt(main_block.style.height.replace('px', ''),10),
                ...
                if (aX<0){
                    let l = parseInt(mousePosX - this.state.blockX,10);
                    aX = startX - mousePosX;
                    preview_block.style.marginLeft = "-"+(mousePosX - this.state.blockX)+"px";
                ...
                if (aY<0){
                    let t = parseInt(mousePosY - this.state.blockY,10);
                    aY = startY - mousePosY;
                    preview_block.style.marginTop = "-"+(mousePosY - this.state.blockY)+"px";
      ...
      return(
        <div className="RICC_main_container" ref="RICC_main_container" onTouchStart={this.mouseDown}
            ...
            <div className="RICC_crop_block" ref="RICC_crop_block">
                ...
                <div className="RICC_crop_preview"><img src={this.props.src} ref="RICC_crop_preview" alt=""/></div>
            </div>
            <div className="RICC_bg" ref="RICC_bg_block"/>
            <img className="RICC_image" ref="RICC_image" src={this.props.src} alt=""/>
        </div>
      );
    }
}
